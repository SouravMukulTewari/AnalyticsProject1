...
# Regression Model
# doing 70% training data for prediction and Keeping 30% test data for testing
...
Index = sample(x = 1:nrow(Mer_SalesUncapped), size = 0.7*nrow(Mer_SalesUncapped)) # Training Set Selection
Index

# Create Train dataset
Mer_SalesTrainUncapped = Mer_SalesUncapped[Index, ] # Training Set data of the 70 percent data
nrow(Mer_SalesTrainUncapped)  # Training Set no. of row
summary(Mer_SalesTrainUncapped) # summary of Training Set

# Create Test dataset
Mer_SalesTestUncapped = Mer_SalesUncapped[-Index, ] # # test Set data of the 30 percent data
nrow(Mer_SalesTestUncapped)
summary(object = Mer_SalesTestUncapped)


# So above training set and test set are created

########################### Modeling #################################

head(Mer_SalesTrainUncapped)
mean(Mer_SalesTrainUncapped$Annual_Sales) # same as yval below in summary
...
CartFullModel = rpart(formula = Annual_Sales ~ . , data = Mer_SalesTrainUncapped[,-1], method = "anova")  # dot here means that it considers all the value other than Annual_Sales # in next [,-1] used to remove the merchandize ID as it is not a significant thing to create a regression upon
CartFullModel
summary(object = CartFullModel) # * at end of any line means that it is a leaf node # yval is the mean sales
...
ptree = prune(CartFullModel, cp=0.017) # pruning such that we can reduce the previous overfit model
rpart.plot(ptree)
...
# Predict on testset
CartFullModelPredictTest = predict(CartFullModel, newdata = Mer_SalesTestUncapped, type = "vector") # CartFullModelPredictTest is the Annual_Sales form our developed model taking input of the test data
...
UncappedModelAccuarcy  # error valus i.e difference between the modeled and actual Annual_Sales
# Create 2, 3 Models and then our RMSC i.e root mean quare values , where the value of RMSC value would be less that model would best suit
